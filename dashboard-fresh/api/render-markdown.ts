/**
 * Markdown rendering utilities for Fresh + Deno
 *
 * Renders markdown files with frontmatter and sanitization
 */
...
export async function renderMarkdownFile(
  ...
  if (cached && cached.mtimeMs === (stats.mtime?.getTime() || 0)) {
...
export function invalidateMarkdownCache(requestPath: string) {
  try {
    ...
    console.warn('[render-markdown] Failed to invalidate cache:', error)
  }
}
